namespace DiversityService.API.Test
{
    ...
    public class MappingTest : TestBase
    {
        ...
        public class TaxonList : MappingTest
        {
            [Fact]
            public void ToModel()
            {
                // Arrange
                var list = new DB.TaxonNames.TaxonList() { DataSource = "TestSource", DisplayText = "TestText", ListID = 100, TaxonomicGroup = "plant" };

                // Act
                var mList = Mapper.Map<Model.Internal.ServerTaxonList>(list);

                // Assert
                // Internal Properties
                Assert.Equal(list.ListID, mList.DatabaseId);
                Assert.Null(mList.Module);
                // Public Properties
                Assert.Equal(list.DisplayText, mList.Name);
                Assert.Equal(list.TaxonomicGroup, mList.TaxonGroup);
                // The id should be the SHA1 hash of a string generated by the following template:
                // {Id}_{DataSource}_{DisplayText}_{TaxonGroup}
                // SHA1("100_TestSource_TestText_plant")
                Assert.Equal("b6087dfd2dd1e000935e9cbc2f2d2e4b1c8eb63a", mList.Id, StringComparer.InvariantCultureIgnoreCase);
            }
        }

        public class TaxonName : MappingTest
        {
            [Fact]
            public void ToModel()
            {
                // Arrange
                var name = new DB.TaxonNames.TaxonName()
                {
                    AcceptedNameCache = "NameCache",
                    AcceptedNameURI = "ANameURI",
                    Family = "Fam",
                    GenusOrSupragenericName = "genorsup",
                    InfraspecificEpithet = "infra",
                    NameURI = "nameuri",
                    Order = "order",
                    SpeciesEpithet = "spec",
                    Synonymy = "accepted",
                    TaxonNameCache = "tnamec",
                    TaxonNameSinAuthors = "sinauth"
                };

                // Act
                var mName = Mapper.Map<Model.TaxonName>(name);

                // Assert

                Assert.Equal(name.AcceptedNameCache, mName.AcceptedNameCache);
                Assert.Equal(name.AcceptedNameURI, mName.AcceptedNameURI);
                //Assert.Equal(name.Family, mName.Family);
                Assert.Equal(name.GenusOrSupragenericName, mName.GenusOrSupragenic);
                Assert.Equal(name.InfraspecificEpithet, mName.InfraspecificEpithet);
                Assert.Equal(name.NameURI, mName.URI);
                //Assert.Equal(name.Order, mName.Order);
                Assert.Equal(name.SpeciesEpithet, mName.SpeciesEpithet);
                Assert.Equal(name.Synonymy, mName.Synonymy);
                Assert.Equal(name.TaxonNameSinAuthors, mName.TaxonNameSinAuth);
            }
        }
    }
}